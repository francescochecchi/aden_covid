...
### Bespoke functions
  ...
  ## Wrapper function to perform diagnostics and display fit results  
    f_do <- function(f_fit, f_diag_ols, f_cv, f_k_folds, f_plot, f_do_cv) {
  ...
  #...................................  
  ## Function to plot model predictions vs. observations for each cemetery, overall and for a specific date range
    f_plot_gof <- function(f_fit, f_data, f_date_range) {
      ...
      x1[, "counterfactuals"] <- predict(f_fit, newdata = x3, type = "response")
        # back-transform if needed
        x2 <- colnames(model.frame(f_fit))[1]
        if (grepl("log", x2) | grepl("ln", x2) ) {
            x1[, "predictions"] <- exp(x1[, "predictions"]) ;
            x1[, "counterfactuals"] <- exp(x1[, "counterfactuals"])
        }
      ...
      x2 <- seq(f_date_range[1], f_date_range[2], by = 1)
  ...
  ## Function to plot linear interpolation or smooth spline and observations for each single cemetery
    f_plot_spline <- function(f_spline, f_data) {

      # prepare data
      x1 <- as.data.frame(cbind(f_spline$y, f_data) )
      colnames(x1)[1] <- "predictions"
